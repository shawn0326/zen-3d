...
function GLTFParser(json, extensions, options) {
	...
	this.textureLoader = new ImageLoader(this.options.manager);
 ...
 * zen3d {Promise<Object<Array<Object>>>}
 */
...
GLTFParser.prototype.loadScene = function() {
	...
	function buildNodeHierachy(nodeId, parentObject, json, allNodes, skins) {
		...
		if (nodeDef.skin !== undefined) {
			var meshes = node.type === OBJECT_TYPE.GROUP ? node.children : [node];
			...
			for (var i = 0, il = meshes.length; i < il; i++) {
				...
				mesh.bind(new Skeleton(bones), mesh.worldMatrix);
			}
		}
 ...
 * zen3d {Promise<zen3d.KeyframeClip>}
 */
GLTFParser.prototype.loadAnimation = function(animationIndex) {
	...
	return this.getMultiDependencies([
		...
		var clip = new KeyframeClip(name);
		clip.tracks = tracks;
 ...
 * zen3d {Promise<zen3d.Group|zen3d.Mesh|zen3d.SkinnedMesh>}
 */
	...
	return this.getMultiDependencies([
		...
		return scope.loadGeometries(primitives).then(function(geometries) {
			for (var i = 0, il = primitives.length; i < il; i++) {
				...
				// 	geometry.addAttribute('a_Uv2', new BufferAttribute(geometry.attributes.a_Uv.array, 2));
				// }
				...
				if (useFlatShading) {
					material.shading = SHADING_TYPE.FLAT_SHADING;
				}
				...
				if (primitive.mode === WEBGL_CONSTANTS.TRIANGLES ||
					...
					if (primitive.mode === WEBGL_CONSTANTS.TRIANGLE_STRIP) {
						material.drawMode = DRAW_MODE.TRIANGLE_STRIP;
					} else if (primitive.mode === WEBGL_CONSTANTS.TRIANGLE_FAN) {
						material.drawMode = DRAW_MODE.TRIANGLE_FAN;
					}
 ...
 * zen3d {Promise<Array<zen3d.Geometry>>}
 */
 ...
 * zen3d {Promise<zen3d.BufferAttribute|zen3d.InterleavedBufferAttribute>}
 */
...
GLTFParser.prototype.loadMaterial = function(materialIndex) {
	...
	if (materialExtensions[EXTENSIONS.KHR_MATERIALS_PBR_SPECULAR_GLOSSINESS]) {
		...
		materialParams.diffuse = new Color3(1.0, 1.0, 1.0);
	...
	if (materialDef.doubleSided === true) {
		materialParams.side = DRAW_SIDE.DOUBLE;
	}
	...
	if (materialDef.emissiveFactor !== undefined) {
		materialParams.emissive = new Color3().fromArray(materialDef.emissiveFactor);
	}
	...
	return Promise.all(pending).then(function() {
		...
		{ if (material.diffuseMap) material.diffuseMap.encoding = TEXEL_ENCODING_TYPE.SRGB; }
		if (material.emissiveMap) material.emissiveMap.encoding = TEXEL_ENCODING_TYPE.SRGB;
...
GLTFParser.prototype.loadTexture = function(textureIndex) {
	...
	return Promise.resolve(sourceURI).then(function(sourceURI) {
		...
		var texture = Texture2D.fromImage(image);
		...
		texture.format = textureDef.format !== undefined ? WEBGL_TEXTURE_FORMATS[textureDef.format] : WEBGL_PIXEL_FORMAT.RGBA;

		if (textureDef.internalFormat !== undefined && texture.format !== WEBGL_TEXTURE_FORMATS[textureDef.internalFormat]) {
			console.warn('zen3d.GLTFLoader: js does not support texture internalFormat which is different from texture format. ' +
				'internalFormat will be forced to be the same value as format.');
		}

		texture.type = textureDef.type !== undefined ? WEBGL_TEXTURE_DATATYPES[textureDef.type] : WEBGL_PIXEL_TYPE.UNSIGNED_BYTE;

		var samplers = json.samplers || {};
		var sampler = samplers[textureDef.sampler] || {};

		texture.magFilter = WEBGL_FILTERS[sampler.magFilter] || WEBGL_TEXTURE_FILTER.LINEAR;
		texture.minFilter = WEBGL_FILTERS[sampler.minFilter] || WEBGL_TEXTURE_FILTER.LINEAR_MIPMAP_LINEAR;
		texture.wrapS = WEBGL_WRAPPINGS[sampler.wrapS] || WEBGL_TEXTURE_WRAP.REPEAT;
		texture.wrapT = WEBGL_WRAPPINGS[sampler.wrapT] || WEBGL_TEXTURE_WRAP.REPEAT;
...
GLTFMaterialsPbrSpecularGlossinessExtension.prototype.extendParams = function (materialParams, material, parser) {
	...
	materialParams.specular = new Color3(1.0, 1.0, 1.0);
...
function GLTFLightsExtension(json) {
	...
	for (var lightId in lights) {
		...
		var color = new Color3().fromArray(light.color);
...
var WEBGL_TEXTURE_FORMATS = {
	6406: WEBGL_PIXEL_FORMAT.ALPHA,
	6407: WEBGL_PIXEL_FORMAT.RGB,
	6408: WEBGL_PIXEL_FORMAT.RGBA,
	6409: WEBGL_PIXEL_FORMAT.LUMINANCE,
	6410: WEBGL_PIXEL_FORMAT.LUMINANCE_ALPHA
};

var WEBGL_TEXTURE_DATATYPES = {
	5121: WEBGL_PIXEL_TYPE.UNSIGNED_BYTE,
	32819: WEBGL_PIXEL_TYPE.UNSIGNED_SHORT_4_4_4_4,
	32820: WEBGL_PIXEL_TYPE.UNSIGNED_SHORT_5_5_5_1,
	33635: WEBGL_PIXEL_TYPE.UNSIGNED_SHORT_5_6_5
};

var WEBGL_FILTERS = {
	9728: WEBGL_TEXTURE_FILTER.NEAREST,
	9729: WEBGL_TEXTURE_FILTER.LINEAR,
	9984: WEBGL_TEXTURE_FILTER.NEAREST_MIPMAP_NEAREST,
	9985: WEBGL_TEXTURE_FILTER.LINEAR_MIPMAP_NEAREST,
	9986: WEBGL_TEXTURE_FILTER.NEAREST_MIPMAP_LINEAR,
	9987: WEBGL_TEXTURE_FILTER.LINEAR_MIPMAP_LINEAR
};

var WEBGL_WRAPPINGS = {
	33071: WEBGL_TEXTURE_WRAP.CLAMP_TO_EDGE,
	33648: WEBGL_TEXTURE_WRAP.MIRRORED_REPEAT,
	10497: WEBGL_TEXTURE_WRAP.REPEAT
};
