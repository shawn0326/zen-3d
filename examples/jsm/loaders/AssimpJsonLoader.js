...
AssimpJsonLoader.prototype.parseSkeleton = function(meshName, bonesInfo, nodeTree, boneMap) {
	...
	// generate skeleton
	skeleton = new Skeleton(allbones);
	...
	for (var key in json.properties) {
		...
		if (prop.key === '$tex.file') {
			...
			if (prop.semantic == 1) {
				...
				diffuseMap = Texture2D.fromSrc(material_url);
				// TODO: read texture settings from assimp.
				// Wrapping is the default, though.
				diffuseMap.wrapS = diffuseMap.wrapT = WEBGL_TEXTURE_WRAP.REPEAT;
				...
				var material_url = this.texturePath + prop.value;
				material_url = material_url.replace(/.\\/g, '');
				normalMap = Texture2D.fromSrc(material_url);
				// TODO: read texture settings from assimp.
				// Wrapping is the default, though.
				normalMap.wrapS = normalMap.wrapT = WEBGL_TEXTURE_WRAP.REPEAT;
			}
...
AssimpJsonLoader.prototype.parseMesh = function(json) {
	...
	if (bones) {
		var buffer = new InterleavedBuffer(new Float32Array(g_v), 19);
		var attribute;
		...
		attribute = new InterleavedBufferAttribute(buffer, 4, 9);
		geometry.addAttribute("skinIndex", attribute);
		attribute = new InterleavedBufferAttribute(buffer, 4, 15);
		geometry.addAttribute("skinWeight", attribute);
		attribute = new InterleavedBufferAttribute(buffer, 2, 13);
		geometry.addAttribute("a_Uv", attribute);
	} else {
		var buffer = new InterleavedBuffer(new Float32Array(g_v), 17);
		var attribute;
		attribute = new InterleavedBufferAttribute(buffer, 3, 0);
		geometry.addAttribute("a_Position", attribute);
		attribute = new InterleavedBufferAttribute(buffer, 3, 3);
