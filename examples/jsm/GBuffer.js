...
var GBuffer = (function() {
	...
	function GBuffer(width, height) {
		...
		this._renderTarget1.texture.minFilter = WEBGL_TEXTURE_FILTER.NEAREST;
		this._renderTarget1.texture.magFilter = WEBGL_TEXTURE_FILTER.NEAREST;
		this._renderTarget1.texture.type = WEBGL_PIXEL_TYPE.HALF_FLOAT;
		...
		this._depthTexture.type = WEBGL_PIXEL_TYPE.UNSIGNED_INT_24_8; // higher precision for depth
		this._depthTexture.format = WEBGL_PIXEL_FORMAT.DEPTH_STENCIL;
		this._depthTexture.magFilter = WEBGL_TEXTURE_FILTER.NEAREST;
		this._depthTexture.minFilter = WEBGL_TEXTURE_FILTER.NEAREST;
		...
		this._texture2.minFilter = WEBGL_TEXTURE_FILTER.LINEAR;
		this._texture2.magFilter = WEBGL_TEXTURE_FILTER.LINEAR;
		this._texture2.generateMipmaps = false;

		this._useMRT = false;

		this._renderTarget2 = new RenderTarget2D(width, height);
		this._renderTarget2.texture.minFilter = WEBGL_TEXTURE_FILTER.LINEAR;
		this._renderTarget2.texture.magFilter = WEBGL_TEXTURE_FILTER.LINEAR;
			...
			if (glCore.capabilities.version >= 2 || glCore.capabilities.getExtension('WEBGL_draw_buffers')) {
				if (!this._useMRT) {
					...
					if (glCore.capabilities.version >= 2) {
						...
						if (ext) {
							this._renderTarget1.texture.internalformat = WEBGL_PIXEL_FORMAT.RGBA32F;
							this._renderTarget1.texture.type = WEBGL_PIXEL_TYPE.FLOAT;
							// this._renderTarget1.texture.internalformat = WEBGL_PIXEL_FORMAT.RGBA16F;
							// this._renderTarget1.texture.type = WEBGL_PIXEL_TYPE.HALF_FLOAT;
						} else {
							this._renderTarget1.texture.type = WEBGL_PIXEL_TYPE.UNSIGNED_BYTE;
						}

						this._depthTexture.internalformat = WEBGL_PIXEL_FORMAT.DEPTH24_STENCIL8;
						this._depthTexture.type = WEBGL_PIXEL_TYPE.UNSIGNED_INT_24_8;

						// this._depthTexture.internalformat = WEBGL_PIXEL_FORMAT.DEPTH32F_STENCIL8;
