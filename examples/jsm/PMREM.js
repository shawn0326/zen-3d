...
var PMREM = (function() {
	...
	function generateNormalDistribution(capabilities, roughnessLevels, sampleSize) {
		...
		normalDistribution.type = WEBGL_PIXEL_TYPE.FLOAT;
		if (capabilities.version >= 2) {
			normalDistribution.internalformat = WEBGL_PIXEL_FORMAT.RGBA32F;
			normalDistribution.format = WEBGL_PIXEL_FORMAT.RGBA;
		}
		normalDistribution.minFilter = WEBGL_TEXTURE_FILTER.NEAREST;
		normalDistribution.magFilter = WEBGL_TEXTURE_FILTER.NEAREST;
	...
	return {
		...
		prefilterEnvironmentMap: function(glCore, envMap, textureOpts) {
			...
			var ArrayCtor = textureType === WEBGL_PIXEL_TYPE.UNSIGNED_BYTE ? Uint8Array : Float32Array;

			var prefilteredCubeMap = new TextureCube();
			prefilteredCubeMap.type = textureType;
			prefilteredCubeMap.generateMipmaps = false;

			var target = new RenderTargetCube(width, height);
			...
			var envMapPass = new EnvironmentMapPass(target);

			var dummyScene = new Scene();

			var geometry = new CubeGeometry(1, 1, 1);
			var material = new ShaderMaterial(prefilterShader);
			material.side = DRAW_SIDE.BACK;
			var skyEnv = new Mesh(geometry, material);
			...
			for (var i = 0; i < mipmapNum; i++) {
				...
				for (var j = 0; j < 6; j++) {
					...
					glCore.gl.readPixels(
						...
						WEBGL_PIXEL_FORMAT.RGBA, textureType, pixels
					);
					if (i === 0) {
