 ...
 */
zen3d.DRACOLoader = function(manager) {
	this.timeLoaded = 0;
...
zen3d.DRACOLoader.prototype = {

	constructor: zen3d.DRACOLoader,

	load: function(url, onLoad, onProgress, onError) {
		var scope = this;
		var loader = new zen3d.FileLoader(scope.manager);
     ...
     *      zen3d.DRAW_MODE.TRIANGLE_STRIP
     */
	...
	decodeDracoFile: function(rawBuffer, callback, attributeUniqueIdMap,
		attributeTypeMap) {
		var scope = this;
		zen3d.DRACOLoader.getDecoderModule()
	...
	addAttributeToGeometry: function(dracoDecoder, decoder, dracoGeometry,
		...
		geometry.addAttribute(attributeName,
			new zen3d.BufferAttribute(geometryBuffer[attributeName],
				numComponents));
	...
	convertDracoGeometryToZen3d: function(dracoDecoder, decoder, geometryType,
		...
		if (geometryType == dracoDecoder.TRIANGULAR_MESH) {
			if (this.drawMode === zen3d.DRAW_MODE.TRIANGLE_STRIP) {
		...
		if (geometryType == dracoDecoder.TRIANGULAR_MESH) {
			geometry.setIndex(geometryBuffer.indices.length > 65535 ?
				new zen3d.BufferAttribute(new Uint16Array(geometryBuffer.indices), 1) :
				new zen3d.BufferAttribute(new Uint32Array(geometryBuffer.indices), 1)
			);
		}
...
zen3d.DRACOLoader.decoderPath = './';
zen3d.DRACOLoader.decoderConfig = {};
zen3d.DRACOLoader.decoderModulePromise = null;

/**
 * Sets the base path for decoder source files.
 * @param {string} path
 */
zen3d.DRACOLoader.setDecoderPath = function (path) {
	zen3d.DRACOLoader.decoderPath = path;
};
 ...
 * @param {Object} config
 */
zen3d.DRACOLoader.setDecoderConfig = function (config) {
	var wasmBinary = zen3d.DRACOLoader.decoderConfig.wasmBinary;
	zen3d.DRACOLoader.decoderConfig = config || {};
	zen3d.DRACOLoader.releaseDecoderModule();

	// Reuse WASM binary.
	if (wasmBinary) zen3d.DRACOLoader.decoderConfig.wasmBinary = wasmBinary;
};
 ...
 */
zen3d.DRACOLoader.releaseDecoderModule = function () {
	zen3d.DRACOLoader.decoderModulePromise = null;
};
 ...
 */
zen3d.DRACOLoader.getDecoderModule = function () {
	var scope = this;
	var path = zen3d.DRACOLoader.decoderPath;
	var config = zen3d.DRACOLoader.decoderConfig;
	var promise = zen3d.DRACOLoader.decoderModulePromise;
	...
	if (typeof DracoDecoderModule !== 'undefined') {
		...
		// Load with asm.js.
		promise = zen3d.DRACOLoader._loadScript(path + 'draco_decoder.js');
	} else {
		// Load with WebAssembly.
		config.wasmBinaryFile = path + 'draco_decoder.wasm';
		promise = zen3d.DRACOLoader._loadScript(path + 'draco_wasm_wrapper.js')
			.then(function () {
				return zen3d.DRACOLoader._loadArrayBuffer(config.wasmBinaryFile);
			})
	...
	zen3d.DRACOLoader.decoderModulePromise = promise;
	return promise;
};
 ...
 * @return {Promise}
 */
zen3d.DRACOLoader._loadScript = function (src) {
 ...
 * @return {Promise}
 */
zen3d.DRACOLoader._loadArrayBuffer = function (src) {
